...
class FaceParser(nn.Module):
    def __init__(self, ckpt_path, device):
        ...
        self.netSeg.load_state_dict(torch.load(ckpt_path, weights_only=True))
        self.netSeg.eval()

    def netseg(self, image):
        img_seg = self.netSeg(self.spNorm(image))[0]
        return img_seg

    def makeMask(self, origin_mask):
        origin_mask = origin_mask.squeeze(0).detach()
        _, tensor = torch.max(origin_mask, dim=0)
        
        
        ids = torch.tensor([1, 2, 3, 4, 5, 10, 11, 12, 13], device=origin_mask.device)
    ...
    def mask(self, image_path):
        ...
        img_seg = self.netSeg(self.spNorm(image.to(self.device)))[0]
        mask  = self.makeMask(img_seg)
        return mask
    
class SpecificNorm(nn.Module):
    def __init__(self):
        super(SpecificNorm, self).__init__()
        self.mean = torch.tensor([0.5, 0.5, 0.5], dtype=torch.float32).view(1, 3, 1, 1).cuda()
        self.std = torch.tensor([0.5, 0.5, 0.5], dtype=torch.float32).view(1, 3, 1, 1).cuda()

    def forward(self, tensor):
        mean = self.mean.expand_as(tensor)
        std = self.std.expand_as(tensor)
...
def show_editied_masked_image(
    ...
    fig.add_subplot(rows, cols, fig_idx)
    fig_idx += 1
    _set_image_plot_name("Target Image")
    ...
    fig.add_subplot(rows, cols, fig_idx)
    _set_image_plot_name("Swapped Image")
...
class ImageAugmentations(nn.Module):
    def __init__(self, output_size, augmentations_number, p=0.7):
        super(ImageAugmentations, self).__init__()
